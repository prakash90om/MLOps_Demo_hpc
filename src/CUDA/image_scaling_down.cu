...
__global__ void scale_image(unsigned char* scale_img,unsigned char* img,int height,int width,int scale_factor)
{
	...
	{	
		scale_img[3 * (row * width/scale_factor + col) + 0] = img[3 * (row * width * scale_factor + col * scale_factor) + 0];
		scale_img[3 * (row * width/scale_factor + col) + 1] = img[3 * (row * width * scale_factor + col * scale_factor) + 1];
		scale_img[3 * (row * width/scale_factor + col) + 2] = img[3 * (row * width * scale_factor + col * scale_factor) + 2];
	}
}
...
int main(void) {
	...
	cudaMalloc(( void **)&d_scale_img, sizeof(unsigned char)*img_size);
	...
	cudaMemcpy(scale_img, d_scale_img, sizeof(unsigned char)*img_size, cudaMemcpyDeviceToHost);

	cudaFree(d_img);
	cudaFree(d_scale_img);
	stbi_write_jpg("augmented_images/image_scaling_down.jpg", width/scale_factor, height/scale_factor, channels, scale_img, 100);
